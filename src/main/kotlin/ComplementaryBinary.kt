...
val Int.cb: ComplementaryBinary get() {
    if (this == 0) return ComplementaryBinary(IntArray(BITS_COUNT) { 0 })

    val binary = IntArray(BITS_COUNT)
    var i = BITS_COUNT - 1
    var module = if (this < 0) -this - 1 else this

    while (module != 0) {
        binary[i--] = if (this < 0) 1 - (module and 1) else module and 1
        module = module ushr 1
    }
